...
class Network(nn.Module):
    ...
    def __init__(self, C, data_channel, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3):
        ...
        C_curr = stem_multiplier * C  # 3*16
        self.stem = nn.Sequential(
            nn.Conv2d(self._data_channel, C_curr, 3, padding=1, bias=False),
            nn.BatchNorm2d(C_curr)
        )
    ...
    def new(self):
        model_new = Network(self._C, self._data_channel, self._num_classes, self._layers, self._criterion).cuda()
